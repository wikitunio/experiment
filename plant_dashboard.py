...
def load_data():
    ...
    except: return pd.DataFrame(), "Error in PQ Sheet"
    ...
    except: return pd.DataFrame(), "Error in Efficiencies Sheet"
...
if err_msg:
    ...
    if not daily_data.empty:
        ...
        conv = get_val(daily_data, 'CO2_Conv')
        if conv > 0 and conv <= 1.0: conv *= 100 
        r1.metric("CO2 Conversion (Ref: 58.0%)", f"{conv:.1f} %")
        ...
        with g1: 
            ...
            st.plotly_chart(make_gauge(get_val(daily_data, 'Stripper_Eff')), use_container_width=True, key="g1")
        with g2: 
            st.markdown("<div class='gauge-title'>HPD</div><div class='gauge-sub'>Ref/Design: 65.4%</div>", unsafe_allow_html=True)
            st.plotly_chart(make_gauge(get_val(daily_data, 'HPD_Eff')), use_container_width=True, key="g2")
        with g3: 
            st.markdown("<div class='gauge-title'>LPD</div><div class='gauge-sub'>Ref/Design: 65.0%</div>", unsafe_allow_html=True)
            st.plotly_chart(make_gauge(0), use_container_width=True, key="g3") 
        ...
        df_7d = df[(df['Date'] <= selected_date) & (df['Date'] >= week_start)]
        ...
        t1, t2 = st.columns(2)
        with t1:
            st.plotly_chart(add_ref(px.line(df_7d, x='Date', y='Moisture', markers=True, title='Avg Moisture (Design: 0.3%)'), 0.3), use_container_width=True, key="t1")
            st.plotly_chart(add_ref(px.line(df_7d, x='Date', y='APS', markers=True, title='Avg APS Trend')), use_container_width=True, key="t2")
        with t2:
            st.plotly_chart(add_ref(px.line(df_7d, x='Date', y='Biuret', markers=True, title='Avg Biuret (Design: 0.9%)'), 0.9), use_container_width=True, key="t3")
            st.plotly_chart(add_ref(px.line(df_7d, x='Date', y='Rx_NC', markers=True, title='Reactor N/C (Design: 3.11)'), 3.11), use_container_width=True, key="t4")
    else:
        st.info(f"No data found for {selected_date.strftime('%d %b %Y')}.")
