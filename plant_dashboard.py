...
def load_data():
    ...
    # FIX: Tell Python how to handle numbers vs text
    ...
    for col in df_master.columns:
        if col == 'Date':
            ...
            agg_funcs[col] = 'sum' # Sum daily production
        elif col in numeric_cols:
            agg_funcs[col] = 'mean' # Average the numbers (Lab results, load, etc)
        else:
            agg_funcs[col] = 'first' # Just take the first text value for Remarks
...
try:
    ...
    if not daily_data.empty:
        ...
        with tab1:
            ...
            # Show Remarks if they exist for that day
            remarks = get_val(daily_data, 'Remarks')
            if pd.notna(remarks) and str(remarks).strip() != "0":
                st.info(f"📝 **Shift Remarks:** {remarks}")
